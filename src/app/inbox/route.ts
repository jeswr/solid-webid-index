...
export async function POST(req: Request): Promise<Response> {
  ...
  // Trust the Content-Length when present, but ALSO bound the actually-read bytes (a client may lie
  // or omit it) — read the raw text and reject if it exceeds the cap.
  ...
  // Byte length (not char length) — UTF-8 multibyte chars must count fully.
  if (Buffer.byteLength(bodyText, "utf8") > MAX_BYTES_INBOX) {
  ...
  for (const raw of candidates) {
    ...
    if (!dailyGranted) {
      // Budget exhausted — do not enqueue (anti-amplification global ceiling).
      continue;
    }
  ...
  // ── Dedup-only outcomes (no new admissions) ─────────────────────────────────
  ...
  // Re-serialise a CANONICAL body (never echo the attacker's bytes — DESIGN.md §2.3 M5). Built from
  // the parsed dataset via the house serialiser (never hand-concatenated).
  let canonicalBody: string;
  try {
    const quads: Quad[] = [];
    for (const quad of parsed.dataset) quads.push(quad as Quad);
    canonicalBody = await serializeTurtle(quads);
  } catch {
    canonicalBody = "";
  }
